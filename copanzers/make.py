        ...
        for preset in os.listdir(entity_path):
            with open(os.path.join(entity_path, preset)) as pref:
                try:
                    ...
                    log.error(
                        "Failed to read in preset '%s', is no valid json.",
                        preset)
                    continue
            ...
            try:
                ...
                log.error(
                    "Failed to read in preset '%s', didn't specify a name.",
                    preset)
                continue
            ...
            for s in cp_pre:
                ...
                try:
                    if "." in s:
                        ...
                        # have to get the respective function instead of just
                        # the class
                        c, fac, *_ = s.split(".")
                    ...
                    factory = err.args[0].split("'")[-2]
                    log.error(
                        "Failed to read in preset '%s', specified component \
or factory '%s' doesn't exist.", preset, factory)
    ...
    def make(self, name, *vargs, pos=(0, 0)):
        ...
        for i, va in enumerate(proto["vargs"]):
            try:
                ...
                log.error(
                    "Preset '%s' specified more arguments than were provided.",
                    name)
                return
        del proto["vargs"]
...
the respective elements in 'mounts' to null.", name)
        ...
        if needs_hp_bar:
            ...
            self.eman.add_component(
                e,
                HealthBar((0, -0.8 * size[1]), (hp / 2, 6)))
        return e

    def __getitem__(self, name):
        return functools.partial(self.make, name)
